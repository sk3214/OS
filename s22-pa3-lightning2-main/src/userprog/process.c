  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      // printf("Hash init start\n");
      if(thread_current()->page_table == NULL){
        struct hash *supplemental_hash_table = malloc(sizeof(struct hash));
        thread_current()->page_table = supplemental_hash_table;
        bool yes = hash_init(thread_current()->page_table,&page_hash,&page_less,NULL);
        // init_frame_table();
        // printf("Hash init %d\n",yes);
      }
      struct page_table_entry *pte = create_page(file,ofs,upage,read_bytes,zero_bytes,writable);
      // printf("Back out here?\n");
      // if(!load_from_file(pte)){
      //   return false;
      // }
      ...
      ofs += page_read_bytes; // Watch watch and watch recitation vids!!!
      upage += PGSIZE;
    }
  // create_page(file,ofs,upage,read_bytes,zero_bytes,writable);
